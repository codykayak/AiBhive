...
import {
  AUTOIMMUNE_DISCLAIMER_SECTIONS,
  AUTOIMMUNE_DISCLAIMER_TITLE,
  acceptAutoimmuneDisclaimer,
} from '../../../lib/oregonPlantMedicine/autoimmuneDisclaimer';
...
export default function AutoimmuneDisclaimerModal({ onAccepted, onCancel, reviewOnly = false, userId }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/85 backdrop-blur-sm">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="autoimmune-disclaimer-title"
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 p-4 border-b border-rose-500/20 bg-slate-950/95 backdrop-blur">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <h2 id="autoimmune-disclaimer-title" className="text-lg font-bold text-white mt-0.5">
                {AUTOIMMUNE_DISCLAIMER_TITLE}
              </h2>
            </div>
          </div>
        ...
        <div className="p-5 space-y-4 text-sm text-slate-300 leading-relaxed">
          {AUTOIMMUNE_DISCLAIMER_SECTIONS.map((section) => (
            ...
            <>
              <label className="flex items-start gap-3 rounded-xl border border-slate-700 bg-slate-900/60 p-4 cursor-pointer">
                ...
                <span className="text-sm text-slate-200">
                  I understand this is educational research only. I will not use it as medical advice or delay professional
                  care.
                </span>
              </label>
