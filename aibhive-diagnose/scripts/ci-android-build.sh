...
echo "==> Production API URL for APK"
export EXPO_PUBLIC_API_URL="${EXPO_PUBLIC_API_URL:-https://aibhive.com}"
export EXPO_PUBLIC_GROK_MODEL="${EXPO_PUBLIC_GROK_MODEL:-grok-2-vision-1212}"
echo "EXPO_PUBLIC_API_URL=$EXPO_PUBLIC_API_URL"

echo "==> Checking launcher assets"
for f in assets/brand/aibhive-logo-mark.png; do
  if [[ ! -f "$f" ]]; then
    echo "MISSING: $f"
    exit 1
  fi
...
PUBLIC_APK="../../public/aibhive-diagnose.apk"
cp "$APK_OUT" "$PUBLIC_APK"
echo "==> Diagnose APK ready at public/aibhive-diagnose.apk"
