  ...
  return (
    <ScrollView className="flex-1 bg-hive-bg" contentContainerStyle={{ padding: 20, paddingBottom: 48 }}>
      ...
      {DELTA_T_TARGETS.map((row) => (
        <View key={row.mode} className="mb-2 rounded-2xl border border-hive-border bg-hive-card p-4">
      ...
      {SUPERHEAT_TARGETS.map((row) => (
        <View key={row.refrigerant} className="mb-2 rounded-2xl border border-hive-border bg-hive-card p-4">
      ...
      {FILTER_GUIDE.map((row) => (
        <View key={row.size} className="mb-2 flex-row rounded-2xl border border-hive-border bg-hive-elevated px-4 py-3">
      ...
      {HVAC_CODE_REFS.map((ref) => (
        <View key={ref.id} className="mb-3 rounded-2xl border border-hive-border bg-hive-card p-4">
