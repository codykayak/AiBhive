          ...
          {loading ? (
            ...
            <>
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-emerald-500/20 overflow-hidden shrink-0">
                  {avatarUrl ? (
                    <img src={avatarUrl} alt="" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-xl font-bold text-emerald-300">
                      {(displayName[0] || '?').toUpperCase()}
                    </div>
                  )}
                </div>
                <label className="text-xs font-bold text-emerald-300 cursor-pointer hover:text-emerald-200">
                  <input
                    type="file"
                    accept="image/*"
                    className="sr-only"
