...
import {
  HERBS_DISCLAIMER_SECTIONS,
  HERBS_DISCLAIMER_TITLE,
  acceptHerbsDisclaimer,
} from '../../../lib/oregonPlantMedicine/herbsDisclaimer';
...
export default function HerbsDisclaimerModal({ onAccepted, onCancel, reviewOnly = false, userId }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/85 backdrop-blur-sm">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="herbs-disclaimer-title"
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 p-4 border-b border-amber-500/20 bg-slate-950/95 backdrop-blur">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <h2 id="herbs-disclaimer-title" className="text-lg font-bold text-white mt-0.5">
                {HERBS_DISCLAIMER_TITLE}
              </h2>
            </div>
          </div>
        ...
        <div className="p-5 space-y-4 text-sm text-slate-300 leading-relaxed">
          {HERBS_DISCLAIMER_SECTIONS.map((section) => (
