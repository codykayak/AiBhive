...
function renderMarkdown(md: string) {
  ...
  const flushList = () => {
    ...
    nodes.push(
      <ul key={`ul-${nodes.length}`} className="list-disc pl-5 space-y-1 text-slate-300 text-sm my-3">
        {listItems.map((item) => (
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('# ')) {
      ...
      flushList();
      nodes.push(
        <h2 key={i} className="text-xl font-bold text-amber-300 mt-8 mb-2">
          {line.slice(3)}
        </h2>
      );
    } else if (line.startsWith('### ')) {
      ...
      if (cells.length >= 2) {
        nodes.push(
          <div key={i} className="grid grid-cols-2 gap-2 text-sm border-b border-white/5 py-2">
            <span className="font-semibold text-slate-200">{cells[0]}</span>
      ...
      flushList();
      nodes.push(<hr key={i} className="border-white/10 my-6" />);
    } else if (line.trim()) {
      flushList();
      nodes.push(
...
export default function ProsAdminManualPanel({ user }: Props) {
  ...
  return (
    <div className="max-w-3xl">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 mb-6 flex items-start gap-3">
        <BookOpen className="w-6 h-6 text-sky-400 shrink-0" />
        <div>
          <h2 className="font-bold text-lg">Administrator user manual</h2>
          <p className="text-sm text-slate-400 mt-1">
            Full guide to Pros HQ and the Diagnose field app. The <strong className="text-slate-200">Pros Assistant</strong>{' '}
      ...
      {loading ? (
        ...
        <article className="rounded-2xl border border-white/10 bg-black/20 p-6 sm:p-8">
