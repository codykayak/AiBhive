...
export default function AutoSocialPanel({ user }: AutoSocialPanelProps) {
  ...
  async function handleGenerate(force = false) {
    ...
    try {
      const res = await generatePost(user, force);
      if (res.skipped) showToast(res.reason === 'already_exists' ? 'Today exists — use Regenerate' : 'Already in progress');
      else showToast('Post bundle ready');
      await refresh();
    } catch (e) {
